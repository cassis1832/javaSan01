...
public interface NcmRepository extends JpaRepository<Ncm, String>, JpaSpecificationExecutor<Ncm> {
    ...
    Optional<Ncm> findById(@Nonnull String codNcm);

    @Query("select i from Ncm i Where i.status  = ?1")
    Page<Ncm> pageNcm(int status, Pageable pageable);

    @Query("select i from Ncm i where i.status = ?1 and " +
            "(concat(i.codNcm, i.descricao) like concat('%',?2,'%')) ")
    Page<Ncm> pageNcm(int status, String filterText, Pageable pageable);
}