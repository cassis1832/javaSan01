...
public class UnidMedidaService {
    ...
    public UnidMedida findByCodUnimed(@NotNull final String codUniMed) {
        return unidMedidaRepository.findByCodUniMed(jwtToken.getEmpresa(), codUniMed)
                .orElseThrow(() -> new ApiRequestException("Unidade de medida não encontrada"));
    }

    public ApiResponse listUnidMedida(final int status) {
        return new ApiResponse(true, unidMedidaRepository.listUniMed(jwtToken.getEmpresa(), status));
    }

    @Transactional
    public UnidMedida create(@NotNull final UnidMedida unidMedida) {

        if (unidMedidaRepository.existsBycodUniMed(jwtToken.getEmpresa(), unidMedida.getCodUniMed()))
    ...
    public UnidMedida update(@NotNull final UnidMedida dto) {
        ...
        UnidMedida unidMedida = unidMedidaRepository.findByCodUniMed(jwtToken.getEmpresa(), dto.getCodUniMed())
                .orElseThrow(() -> new ApiRequestException("Unidade de medida não encontrada!"));
    ...
    public void delete(@NotNull final String codUniMed) {
        ...
        if (!unidMedidaRepository.existsBycodUniMed(jwtToken.getEmpresa(), codUniMed))
            throw new ApiRequestException("Unidade de medida não encontrada!");

        unidMedidaRepository.deleteByCodUniMed(jwtToken.getEmpresa(), codUniMed);
    }
}
